...
test_adder_divi()->
    ...
    DnsIpAddr=CInfo#computer_info.ip_addr,
    DnsPort=CInfo#computer_info.port,
    AdderList=tcp_client:call({DnsIpAddr,DnsPort},{dns_service,get,["adder_service"]}),
    DiviList=tcp_client:call({DnsIpAddr,DnsPort},{dns_service,get,["divi_service"]}),
...
start_dns()->
    [{"pod_master",CInfo}]=ets:lookup(?ETS,"pod_master"),
    Vm=CInfo#computer_info.vm,
    VmName=CInfo#computer_info.vm_name,
...
start_computer_pods()->
    ...
    ?assertEqual([{ok,pod_master@asus},
		  {ok,pod_landet_1@asus},
		  {ok,pod_lgh_1@asus},
...
%							])||{VmName,Vm}<-ComputerVmList]),
    ...
    ?assertEqual([{pong,pod_master@asus,lib_service},
		  {pong,pod_landet_1@asus,lib_service},
   ...
   % start tcp server on each computer pod
    ...
    ?assertEqual([ok,ok,ok,ok],[rpc:call(CInfo#computer_info.vm,lib_service,start_tcp_server,
    ...
    ?assertEqual([{pong,pod_master@asus,lib_service},
		 ...
		 [tcp_client:call({CInfo#computer_info.ip_addr,CInfo#computer_info.port},{lib_service,ping,[]})||{CId,CInfo}<-ComputerList]),
...
create_vm_info({VmName,IpAddr,Port,Mode})->
    ...
    {VmName,#computer_info{vm_name=VmName,vm=Vm,ip_addr=IpAddr,port=Port,mode=Mode}}.
